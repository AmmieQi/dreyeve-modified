...
# BATCH GENERATOR CONSTANTS FROM DREYEVE CONFIG
...
def sample_signature(sequences, allowed_frames, image_size, allow_mirror, sequence_ids):
    """
...
def load_batch_data(signatures, nb_frames, image_size, batch_type, sequence_ids):
    """
...
def dreyeve_I_batch(batchsize, nb_frames, image_size, mode, gt_type, sequence_ids):
    ...
    for b in range(0, batchsize):
        signatures.append(sample_signature(sequences=sequences, allowed_frames=allowed_frames,
                                           image_size=image_size, allow_mirror=allow_mirror, sequence_ids=sequence_ids))

    # get an image batch
    I = load_batch_data(signatures=signatures, nb_frames=nb_frames, image_size=image_size, batch_type='image', sequence_ids=sequence_ids)
    # if SIMO_MODE:
